...
def drawCompass(im, videoWidth, videoHeight, heading):
	...
	for i in range(minI, maxI):
		...
		if (i == map(180,0,360,minI, maxI)):
			cv2.line("S", new cv.Point(reduce(i+heading+videoWidth/2,compassRange)+minI, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.35,  (onScreenColorR, onScreenColorG, onScreenColorB))
		if (i == map(90,0,360,minI, maxI)):
			cv2.line("W", new cv.Point(reduce(i+heading+videoWidth/2,compassRange)+minI, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.35,  (onScreenColorR, onScreenColorG, onScreenColorB))
